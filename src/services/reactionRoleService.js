...
export async function getReactionRoleByCustomId(client, guildId, customId) {
 validateGuildId(guildId);
 if (!customId || typeof customId!=="string") return null;
 return await client.db.get(`reaction_role_button:${guildId}:${customId}`);
}
export async function saveReactionRoleButton(client,guildId,customId,roleId){
 validateGuildId(guildId); validateRoleId(roleId);
 await client.db.set(`reaction_role_button:${guildId}:${customId}`,{guildId,customId,roleId});
 return true;
}
export async function deleteReactionRoleButton(client,guildId,customId){
 validateGuildId(guildId);
 await client.db.delete(`reaction_role_button:${guildId}:${customId}`);
 return true;
}
